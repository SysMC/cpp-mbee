...
2. Отладочная плата MSP-EXP430FR4133 семейства LaunchPad.
...
3. Запустить любую терминальную программы с параметрами 9600 8N1.
...
ВНИМАНИЕ! В случае необходимости уменьшить размер кода, требуется подобрать необходимые параметры оптимизации проекта. Для устройств с размером RAM 512 байт, необходимо установить в файле 
ControlCpp-mbeeConfig.dat размеры буферов UART UART_TX_BUFFER_SIZE=32 и UART_RX_BUFFER_SIZE=64. 
